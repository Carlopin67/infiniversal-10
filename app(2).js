...
const EditorView = (() => {
  ...
  /* ── Insertar marcador de sección (no editable) ── */
  songToolbar.querySelectorAll('.section-btn').forEach(btn =>
    btn.addEventListener('click', () => {
      ...
      // Bloque marcador: contenteditable=false → no se puede editar ni borrar con teclado
      const marker = document.createElement('div');
      marker.className = 'song-section-block';
      marker.setAttribute('contenteditable', 'false');
      marker.textContent = btn.dataset.section;

      // Contenedor wrapper
      const wrapper = document.createElement('div');
      wrapper.appendChild(marker);

      // Línea editable debajo para que el cursor se posicione ahí
      const writeLine = document.createElement('div');
      writeLine.innerHTML = '<br>';
      wrapper.appendChild(writeLine);

      range.insertNode(wrapper);

      // Mover cursor a la línea editable
      const newRange = document.createRange();
      newRange.setStart(writeLine, 0);
  ...
  /* ── Long-press para eliminar marcador ── */
  let markerPressTimer = null;
  let markerPressTarget = null;

  function startMarkerPress(e) {
    const block = e.target.closest('.song-section-block');
    if (!block) return;
    e.preventDefault();
    markerPressTarget = block;
    block.classList.add('marker-pressing');
    markerPressTimer = setTimeout(() => {
      block.classList.remove('marker-pressing');
      block.classList.add('marker-confirm-delete');
      block.dataset.confirmDelete = 'true';
      // Auto-cancelar confirmación tras 3s
      setTimeout(() => {
        if (block.dataset.confirmDelete === 'true') {
          block.classList.remove('marker-confirm-delete');
          delete block.dataset.confirmDelete;
        }
      }, 3000);
    }, 600);
  }

  function cancelMarkerPress() {
    clearTimeout(markerPressTimer);
    if (markerPressTarget) {
      markerPressTarget.classList.remove('marker-pressing');
      markerPressTarget = null;
    }
  }

  editor.addEventListener('touchstart', startMarkerPress, { passive: false });
  editor.addEventListener('touchend', cancelMarkerPress);
  editor.addEventListener('touchmove', cancelMarkerPress);

  // Click en marcador con confirmación activa → eliminar
  editor.addEventListener('click', e => {
    ...
    if (block.dataset.confirmDelete === 'true') {
      ...
      block.remove();
      // Limpiar wrapper vacío
      if (parent && parent !== editor && parent.textContent.trim() === '' && !parent.querySelector('.song-section-block')) {
        parent.remove();
      }
      updateStats(); scheduleSave();
    }
  ...
  /* ── Proteger marcadores contra borrado (móvil: beforeinput) ── */
  editor.addEventListener('beforeinput', e => {
    ...
    // Si hay selección que contiene un marcador, bloquear
    if (!range.collapsed) {
      const frag = range.cloneContents();
      if (frag.querySelector && frag.querySelector('.song-section-block')) {
        e.preventDefault(); return;
      }
    }

    // Backspace (deleteContentBackward) al inicio junto a un marcador
    if (e.inputType === 'deleteContentBackward' && range.collapsed) {
      const node = range.startContainer;
      const offset = range.startOffset;
      if (offset === 0) {
        let prev = node.nodeType === 3 ? node.parentNode.previousElementSibling : node.previousElementSibling;
        if (!prev && node.parentNode !== editor) {
          prev = node.parentNode.previousElementSibling;
        }
        if (prev && (prev.classList?.contains('song-section-block') || prev.querySelector?.('.song-section-block'))) {
          e.preventDefault(); return;
        }
      }
    }

    // Delete (deleteContentForward) al final junto a un marcador
    if (e.inputType === 'deleteContentForward' && range.collapsed) {
      ...
      if (offset === len) {
        let next = node.nodeType === 3 ? node.parentNode.nextElementSibling : node.nextElementSibling;
        if (!next && node.parentNode !== editor) {
          next = node.parentNode.nextElementSibling;
        }
        if (next && (next.classList?.contains('song-section-block') || next.querySelector?.('.song-section-block'))) {
          e.preventDefault(); return;
        }
      }
    }

    // Chequear los targetRanges del evento (más fiable en móvil)
    if (e.getTargetRanges) {
      for (const r of e.getTargetRanges()) {
        const container = r.startContainer;
        const endContainer = r.endContainer;
        // Si el rango toca un marcador
        if (container.nodeType === 1 && container.querySelector?.('.song-section-block')) {
          e.preventDefault(); return;
        }
        if (endContainer.nodeType === 1 && endContainer.querySelector?.('.song-section-block')) {
          e.preventDefault(); return;
        }
        // Si el nodo mismo es un marcador
        const checkNode = n => n.nodeType === 1 && n.classList?.contains('song-section-block');
        if (checkNode(container) || checkNode(endContainer)) {
          e.preventDefault(); return;
        }
        // Comprobar padres
        let p = container;
        while (p && p !== editor) {
          if (checkNode(p)) { e.preventDefault(); return; }
          p = p.parentNode;
        }
      }
    }
  ...
  /* ── MutationObserver: restaurar marcadores si el navegador los borra ── */
  let lastSavedMarkers = [];
  function snapshotMarkers() {
    lastSavedMarkers = [...editor.querySelectorAll('.song-section-block')].map(el => ({
      text: el.textContent,
      html: el.outerHTML
    }));
  }
  // Capturar snapshot en cada guardado
  const origScheduleSave = scheduleSave;
