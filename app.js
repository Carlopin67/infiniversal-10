...
const EditorView = (() => {
  ...
  editor.addEventListener('mousedown', startMarkerPress);
  editor.addEventListener('mouseup', cancelMarkerPress);
  editor.addEventListener('mouseleave', cancelMarkerPress);
  ...
  /* ── Proteger marcadores contra Backspace / Delete ── */
  editor.addEventListener('keydown', e => {
    ...
    // Si la selección contiene un marcador, prevenir
    if (!range.collapsed) {
    ...
    // Backspace al inicio de línea justo después de un marcador
    if (e.key === 'Backspace' && range.collapsed) {
      const node = range.startContainer;
      const offset = range.startOffset;
      // Si estamos al inicio de un nodo de texto
      if (offset === 0) {
    ...
    // Delete al final de línea justo antes de un marcador
