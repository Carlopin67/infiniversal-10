...
const EditorView = (() => {
  ...
  function open(noteId) {
    ...
    // Capturar estado de marcadores para protección
    if (typeof captureMarkerState === 'function') captureMarkerState();
    editor.focus();
  }
  ...
  editor.addEventListener('input', () => {
    if (typeof isRestoringMarker !== 'undefined' && isRestoringMarker) return;
  ...
  songToolbar.querySelectorAll('.section-btn').forEach(btn =>
    btn.addEventListener('click', () => {
      ...
      updateStats(); captureMarkerState(); scheduleSave();
    })
  );
  ...
  editor.addEventListener('click', e => {
    ...
    if (block.dataset.confirmDelete === 'true') {
      ...
      // Actualizar snapshot DESPUÉS del borrado intencional
      captureMarkerState();
      updateStats(); scheduleSave();
    }
  });

  /* ── Proteger marcadores contra borrado ── */
  let markerSnapshot = '';
  let markerCount = 0;
  let isRestoringMarker = false;

  function captureMarkerState() {
    markerCount = editor.querySelectorAll('.song-section-block').length;
    markerSnapshot = editor.innerHTML;
  }

  // Comprobar en cada input si se perdió un marcador
  editor.addEventListener('input', () => {
    if (isRestoringMarker) return;
    const currentCount = editor.querySelectorAll('.song-section-block').length;
    if (currentCount < markerCount && markerSnapshot) {
      // Se borró un marcador accidentalmente → restaurar
      isRestoringMarker = true;
      editor.innerHTML = markerSnapshot;
      // Re-forzar contenteditable=false
      editor.querySelectorAll('.song-section-block').forEach(el => {
        el.setAttribute('contenteditable', 'false');
      });
      updateStats();
      isRestoringMarker = false;
      return; // No actualizar snapshot
    }
    // Todo bien, actualizar snapshot
    captureMarkerState();
  });
