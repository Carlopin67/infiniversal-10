...
const EditorView = (() => {
  ...
  function open(noteId) {
    ...
    // Actualizar snapshot de marcadores para protección
    if (typeof markerSnapshot !== 'undefined') markerSnapshot = editor.innerHTML;
    editor.focus();
  }
  ...
  editor.addEventListener('click', e => {
    ...
    if (!block) return;
    if (block.dataset.confirmDelete === 'true') {
      // Pausar observer para borrado intencional
      ...
      // Actualizar snapshot y reconectar observer
      markerSnapshot = editor.innerHTML;
      markerObserver.observe(editor, { childList: true, subtree: true, characterData: true });
      updateStats(); scheduleSave();
    }
  });

  /* ── Proteger marcadores contra borrado (MutationObserver) ── */
  let markerSnapshot = editor.innerHTML;
  let isRestoringMarkers = false;

  function countMarkers(html) {
    const tmp = document.createElement('div');
    tmp.innerHTML = html;
    return tmp.querySelectorAll('.song-section-block').length;
  }

  function updateMarkerSnapshot() {
    markerSnapshot = editor.innerHTML;
  }

  // Guardar snapshot tras cada inserción de marcador y cada guardado
  const _origScheduleSave = scheduleSave;
  scheduleSave = function() {
    updateMarkerSnapshot();
    _origScheduleSave();
  };

  const markerObserver = new MutationObserver(() => {
    if (isRestoringMarkers) return;
    const currentMarkers = editor.querySelectorAll('.song-section-block').length;
    const savedMarkers = countMarkers(markerSnapshot);
    // Si hay menos marcadores que antes → se borró uno, restaurar
    if (currentMarkers < savedMarkers) {
      isRestoringMarkers = true;
      editor.innerHTML = markerSnapshot;
      // Re-aplicar contenteditable=false a todos los marcadores
      editor.querySelectorAll('.song-section-block').forEach(el => {
        el.setAttribute('contenteditable', 'false');
      });
      isRestoringMarkers = false;
      updateStats();
    } else {
      // Actualizar snapshot si no se perdió ningún marcador
      markerSnapshot = editor.innerHTML;
    }
  });

  markerObserver.observe(editor, {
    childList: true,
    subtree: true,
    characterData: true
  });
